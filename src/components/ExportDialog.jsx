...
const ExportDialog = () => {
  ...
  const exportAsPDF = () => {
    ...
    const processNodes = node => {
      if (node.nodeType === Node.TEXT_NODE && node.parentNode) {
        if (
          ...
          !node.parentNode.classList.contains('ql-color-')
        ) {
    ...
    const headingClasses = contentClone.querySelectorAll(
      '.ql-size-large, .ql-size-huge, .ql-header-1, .ql-header-2, .ql-header-3, .ql-header-4, .ql-header-5, .ql-header-6'
    )
      ...
      <DialogContent>
        ...
        <DialogBody>
          <Input
            ...
            mb={4}
          />
          <p>Save your document as a PDF by clicking the button below.</p>
        </DialogBody>
        <DialogFooter>
